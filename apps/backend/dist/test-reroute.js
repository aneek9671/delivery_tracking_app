...
async function main() {
    ...
    const riderSocket = (0, socket_io_client_1.io)(SERVER_URL);
    const customerSocket = (0, socket_io_client_1.io)(SERVER_URL);
    await Promise.all([
    ...
    let errorMessages = [];
    customerSocket.on('location-updated', (loc) => {
        receivedLocations.push(loc);
    });
    customerSocket.on('error', (msg) => {
    ...
    const allValid = receivedLocations.every(loc => loc.latitude >= -90 && loc.latitude <= 90 &&
